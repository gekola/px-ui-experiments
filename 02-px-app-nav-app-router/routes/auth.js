...
router.get('/login', (req, res) => {
  ...
  // TODO: req.query.state should be escaped before sending to preserve ancor and query
  const redirectUri = req.protocol + '://' + req.get('Host') + '/auth_callback';
  res.redirect(`${UAA_SERVER_URL}/oauth/authorize?response_type=code&client_id=${CLIENT_ID}&redirect_uri=${redirectUri}&state=${escape(req.query.state)}`);
});
